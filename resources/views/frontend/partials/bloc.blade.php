...
<div class="bloc-content">
    ...
    @if(!empty($bloc->url) && !empty($bloc->image))
        <a style="float: left; margin-right: 10px;" href="{{ $bloc->url }}" target="_blank">
            <img style="max-width: 160px" src="{{ secure_asset('/files/uploads/'.$bloc->image) }}" alt="">
        </a>
    @elseif(!empty($bloc->image))
        <img style="max-width: 100px; float: left; margin-right: 10px;" src="{{ secure_asset('/files/uploads/'.$bloc->image) }}" alt="">
    @endif

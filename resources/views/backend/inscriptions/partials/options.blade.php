            ...
            @if($input == 'checkbox')
                @foreach($options as $i => $option)
                    <div class="form-group type-choix">
                        <input type="checkbox" class="option-input" name="colloques[{{ $colloque->id }}][options][]" value="{{ $option->id }}" /> &nbsp;{{ $option->title }}
                    </div>
                @endforeach
            @endif
                ...
                @foreach($options as $i => $option)
                    <div class="form-group type-choix">
                        @foreach($options as $option)
                            ...
                            <textarea class="form-control text-input" name="colloques[{{ $colloque->id }}][options][][{{ $option->id }}]"></textarea>
                        @endforeach
                    </div>
                @endforeach
            @endif
                    ...
                    <div class="form-group group-choix type-choix">
                        ...
                        @if(!$option->groupe->isEmpty())
                            @foreach($option->groupe as $groupe)
                                <div class="radio">
                                    <label>
                                        <input type="radio" required class="group-input" name="colloques[{{ $colloque->id }}][groupes][{{ $option->id }}]" value="{{ $groupe->id }}">{{ $groupe->text }}
                                    </label>
                                </div>
                            @endforeach
                        @endif

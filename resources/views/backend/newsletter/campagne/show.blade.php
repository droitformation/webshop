        ...
        <div class="col-md-12">
            @if($campagne->send_at)
                <div class="alert alert-warning" role="alert">
                    <p>Cette newsletter a été envoyé le {{ $campagne->send_at->format('d/m/Y') }} et ne peux plus être envoyé comme test.</p>
                </div>
            @endif
                                ...
                                @foreach($campagne->content as $bloc)
                                    @if(in_array($bloc->type->id ,array_keys(config('newsletter.components'))))
                                        <div class="bloc_rang" id="bloc_rang_{{ $bloc->id }}" data-rel="{{ $bloc->id }}">
                                            {!! view('backend.newsletter.build.edit.'.$bloc->type->partial)->with(['bloc' => $bloc, 'campagne' => $campagne])->__toString() !!}
                                        </div>
                                    @endif
                                @endforeach
                            @endif
                        ...
                        @foreach($blocs as $bloc)
                            @if(in_array($bloc->id ,array_keys(config('newsletter.components'))))
                                <div class="create_bloc" id="create_{{ $bloc->id }}">
                                    {!! view('backend.newsletter.build.create.'.$bloc->template)->with(['bloc' => $bloc, 'campagne' => $campagne])->__toString() !!}
                                </div>
                            @endif
                        @endforeach
                    @endif
                    ...
                    <div class="component-menu">
                        ...
                        <div class="component-bloc">
                            @if(!$blocs->isEmpty())
                                @foreach($blocs as $bloc)
                                    @if(in_array($bloc->id ,array_keys(config('newsletter.components'))))
                                        {!! view('backend.newsletter.build.blocs')->with(array('bloc' => $bloc)) !!}
                                    @endif
                                @endforeach
                            @endif
                        </div>

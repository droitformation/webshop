...
class Registry {
    ...
    public function set($key, $value)
    {
        ...
        if ($baseKey != $searchKey)
        {
            ...
            {
                $level .= '.'.$key;
                (trim($level, '.') == $searchKey) ? \Arr::set($object, trim($level, '.'), $value) : \Arr::set($object, trim($level, '.'), array());
            }
    ...
    protected function fetchKey($key)
    {
        ...
        {
            $keys = explode('.', $key);
            $search = \Arr::except($keys, 0);

            return array(\Arr::get($keys, 0), implode('.', $search));
        }
